# Copyright (c) Meta Platforms, Inc. and affiliates.
# All rights reserved.

# This source code is licensed under the license found in the
# LICENSE file in the root directory of this source tree.
# --------------------------------------------------------
# References:
# timm: https://github.com/rwightman/pytorch-image-models/tree/master/timm
# DeiT: https://github.com/facebookresearch/deit
# --------------------------------------------------------

from functools import partial
from json import encoder

import torch
import torch.nn as nn
import torch.nn.functional as F

#from timm.models.vision_transformer import PatchEmbed, Block
from timm.models.vision_transformer import Block
from util.pos_embed import get_2d_sincos_pos_embed, get_2d_sincos_pos_embed_flexible, get_1d_sincos_pos_embed_from_grid
from util.misc import concat_all_gather
from util.patch_embed import PatchEmbed_new, PatchEmbed_org, PatchEmbed_SSL
from timm.models.swin_transformer import SwinTransformerBlock

# import os
# import sys
# sys.path.append("/home3/sppro/cqchen5/MAE-emovec2-d24-TAP-formant/my_code/models")
from TAPLoss.TAPLoss import AcousticLoss
from audio_to_mel import Audio2Mel
from formant import QualityFeatures, create_formant_bandwidth_mask
from formant5 import process_batch_magnitude

# from configs import get_config
# from vesper import Vesper, init_with_wavlm

import torch
import torch.nn as nn
import torch.nn.functional as F

from transformers import Wav2Vec2FeatureExtractor, HubertModel, Wav2Vec2Model, WavLMModel


class MaskedAutoencoderViT(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(self, img_size=224, patch_size=16, stride=10, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, 
                 audio_exp=False, alpha=0.0, temperature=.2, mode=0, contextual_depth=8,
                 use_custom_patch=False, split_pos=False, pos_trainable=False, use_nce=False, beta=4.0, decoder_mode=0,
                 mask_t_prob=0.6, mask_f_prob=0.5, mask_2d=False,
                 epoch=0, no_shift=False,
                 ):
        super().__init__()

        self.audio_exp=audio_exp
        self.embed_dim = embed_dim
        self.decoder_embed_dim = decoder_embed_dim
        # --------------------------------------------------------------------------
        # MAE encoder specifics
        if use_custom_patch:
            print(f'Use custom patch_emb with patch size: {patch_size}, stride: {stride}')
            self.patch_embed = PatchEmbed_new(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, stride=stride)
        else:
            self.patch_embed = PatchEmbed_org(img_size, patch_size, in_chans, embed_dim)
            self.patch_embed_ssl = PatchEmbed_SSL((512, 768), (8, 48), in_chans, embed_dim)
        self.use_custom_patch = use_custom_patch
        num_patches = self.patch_embed.num_patches
        num_patches_ssl = self.patch_embed_ssl.num_patches

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.cls_token_ssl = nn.Parameter(torch.zeros(1, 1, embed_dim))

        #self.split_pos = split_pos # not useful
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim), requires_grad=pos_trainable)  # fixed sin-cos embedding
        self.pos_embed_ssl = nn.Parameter(torch.zeros(1, num_patches_ssl + 1, embed_dim), requires_grad=pos_trainable)

        self.encoder_depth = depth
        self.contextual_depth = contextual_depth

        # self.blocks_Mag = nn.ModuleList([
        #     Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
        #     for i in range(4)])
        
        # self.blocks1 = nn.ModuleList([
        #     Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
        #     for i in range(int(depth))])

        self.blocks_Mag = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
            for i in range(4)])
        
        self.blocks1 = nn.ModuleList([
            Block(embed_dim*2, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
            for i in range(int(depth/2))])
        
        self.dowmsample = nn.Linear(embed_dim*2, embed_dim, bias=True)

        self.blocks2 = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
            for i in range(int(depth/2))])
        
        self.norm = norm_layer(embed_dim)

        # --------------------------------------------------------------------------
        # MAE decoder specifics
        self.decoder_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)

        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, decoder_embed_dim), requires_grad=pos_trainable)  # fixed sin-cos embedding

        self.no_shift=no_shift

        self.decoder_mode = decoder_mode
        if self.use_custom_patch: # overlapped patches as in AST. Similar performance yet compute heavy
            window_size= (6,6)
            feat_size = (102,12)
        else:
            window_size= (4,4)
            feat_size = (64,16)  #(64,8)              
        if self.decoder_mode == 1:
            decoder_modules = []
            for index in range(16):
                if self.no_shift:
                    shift_size = (0,0)
                else:
                    if (index % 2) == 0:
                        shift_size = (0,0)
                    else:
                        shift_size = (2,0)
                    #shift_size = tuple([0 if ((index % 2) == 0) else w // 2 for w in window_size])
                decoder_modules.append(
                    SwinTransformerBlock(
                        dim=decoder_embed_dim,
                        num_heads=16,
                        feat_size=feat_size,
                        window_size=window_size,
                        shift_size=shift_size,
                        mlp_ratio=mlp_ratio,
                        drop=0.0,
                        drop_attn=0.0,
                        drop_path=0.0,
                        extra_norm=False,
                        sequential_attn=False,
                        norm_layer=norm_layer, #nn.LayerNorm,
                    )
                )
            self.decoder_blocks = nn.ModuleList(decoder_modules)        
        else:
            # Transfomer
            self.decoder_blocks = nn.ModuleList([
                Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
                for i in range(decoder_depth)])

        self.decoder_norm = norm_layer(decoder_embed_dim)
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True) # decoder to patch

        # --------------------------------------------------------------------------

        self.norm_pix_loss = norm_pix_loss

        self.patch_size=patch_size
        self.stride=stride

        # audio exps
        self.alpha = alpha
        self.T = temperature
        self.mode = mode
        self.use_nce = use_nce
        self.beta = beta

        self.log_softmax=nn.LogSoftmax(dim=-1)

        self.mask_t_prob=mask_t_prob
        self.mask_f_prob=mask_f_prob
        self.mask_2d=mask_2d

        self.epoch = epoch

        # self.channel_proj = nn.Linear(embed_dim * 2, embed_dim)


        self.initialize_weights()

        # init = torch.tensor([1/2, 1/2], dtype=torch.float)
        # self.logits = nn.Parameter(init)  # 可学习参数

        self.TAPloss = AcousticLoss("l2", "/train20/sppro/permanent/cqchen5/AudioMAE/AudioMAE-TAP/TAPLoss/TAP_estimator_model_v0.pt")
        self.TAPloss2 = AcousticLoss("frame_energy_weighted_l2", "/train20/sppro/permanent/cqchen5/AudioMAE/AudioMAE-TAP/TAPLoss/TAP_estimator_model_v0.pt")

        self.num_layers = 8
        # 初始化为全 0，经过 Softmax 后初始权重均为 1/25
        self.layer_weights = nn.Parameter(torch.zeros(self.num_layers))

        self.emo2vec = AutoModel(model="/raw22/asrprg/permanent/glzhong/audio_understand_LLM/1.models/emotion2vec/emotion2vec", disable_update=True)
        self.emo2vec.model.eval()
        for p in self.emo2vec.model.parameters():
            p.requires_grad = False

        self.emo2vec_target = AutoModel(model="/raw22/asrprg/permanent/glzhong/audio_understand_LLM/1.models/emotion2vec/emotion2vec", disable_update=True)
        self.emo2vec_target.model.eval()
        for p in self.emo2vec_target.model.parameters():
            p.requires_grad = False

        model_name = "/train20/sppro/permanent/cqchen5/model/WavLM_Large"
        self.feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(model_name)
        self.model = WavLMModel.from_pretrained(model_name)
        self.model.eval()


    def initialize_weights(self):
        # initialization
        # initialize (and freeze) pos_embed by sin-cos embedding
        if self.audio_exp:
            pos_embed = get_2d_sincos_pos_embed_flexible(self.pos_embed.shape[-1], self.patch_embed.patch_hw, cls_token=True) 
            pos_embed_ssl = get_2d_sincos_pos_embed_flexible(self.pos_embed_ssl.shape[-1], self.patch_embed_ssl.patch_hw, cls_token=True)    
        else:
            pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), cls_token=True)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))
        self.pos_embed_ssl.data.copy_(torch.from_numpy(pos_embed_ssl).float().unsqueeze(0))

        if self.audio_exp:   
            decoder_pos_embed = get_2d_sincos_pos_embed_flexible(self.decoder_pos_embed.shape[-1], self.patch_embed.patch_hw, cls_token=True)
        else:
            decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), cls_token=True)
        self.decoder_pos_embed.data.copy_(torch.from_numpy(decoder_pos_embed).float().unsqueeze(0))

        # initialize patch_embed like nn.Linear (instead of nn.Conv2d)
        w = self.patch_embed.proj.weight.data
        w_ssl = self.patch_embed_ssl.proj.weight.data
        torch.nn.init.xavier_uniform_(w.view([w.shape[0], -1]))
        torch.nn.init.xavier_uniform_(w_ssl.view([w_ssl.shape[0], -1]))

        # timm's trunc_normal_(std=.02) is effectively normal_(std=0.02) as cutoff is too big (2.)
        torch.nn.init.normal_(self.cls_token, std=.02)
        torch.nn.init.normal_(self.mask_token, std=.02)

        torch.nn.init.normal_(self.cls_token_ssl, std=.02)

        # initialize nn.Linear and nn.LayerNorm
        self.apply(self._init_weights)

    def _init_weights(self, m):
        # if (str(m) == "Linear(in_features=512, out_features=256, bias=True)") or (str(m) == "Linear(in_features=256, out_features=25, bias=True)"):
        #     print("SKIP:", m)
        #     return
        if isinstance(m, nn.Linear):
            # we use xavier_uniform following official JAX ViT:
            torch.nn.init.xavier_uniform_(m.weight)
            if isinstance(m, nn.Linear) and m.bias is not None:
                nn.init.constant_(m.bias, 0)
        elif isinstance(m, nn.LayerNorm):
            nn.init.constant_(m.bias, 0)
            nn.init.constant_(m.weight, 1.0)

    def patchify(self, imgs):
        """
        imgs: (N, 3, H, W)
        x: (N, L, patch_size**2 *3)
        L = (H/p)*(W/p)
        """
        p = self.patch_embed.patch_size[0]
        #assert imgs.shape[2] == imgs.shape[3] and imgs.shape[2] % p == 0
        
        if self.audio_exp:
            if self.use_custom_patch: # overlapped patch
                h,w = self.patch_embed.patch_hw
                # todo: fixed h/w patch size and stride size. Make hw custom in the future
                x = imgs.unfold(2, self.patch_size, self.stride).unfold(3, self.patch_size, self.stride) # n,1,H,W -> n,1,h,w,p,p
                x = x.reshape(shape=(imgs.shape[0], h*w, p**2 * 1))
                #x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p))
                #x = torch.einsum('nchpwq->nhwpqc', x)
                #x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 1))
            else:
                h = imgs.shape[2] // p
                w = imgs.shape[3] // p
                #h,w = self.patch_embed.patch_hw
                x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p))
                x = torch.einsum('nchpwq->nhwpqc', x)
                x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 1))
        else:
            h = w = imgs.shape[2] // p
            x = imgs.reshape(shape=(imgs.shape[0], 3, h, p, w, p))
            x = torch.einsum('nchpwq->nhwpqc', x)
            x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 3))

        return x
    
    def patchify_fbank(self, imgs):
        """
        imgs: (N, 3, H, W)
        x: (N, L, patch_size**2 *3)
        L = (H/p)*(W/p)
        """
        p = self.patch_embed_fbank.patch_size[0]
        p2 = self.patch_embed_fbank.patch_size[1]
        #assert imgs.shape[2] == imgs.shape[3] and imgs.shape[2] % p == 0
        
        if self.audio_exp:
            if self.use_custom_patch: # overlapped patch
                h,w = self.patch_embed.patch_hw
                # todo: fixed h/w patch size and stride size. Make hw custom in the future
                x = imgs.unfold(2, self.patch_size, self.stride).unfold(3, self.patch_size, self.stride) # n,1,H,W -> n,1,h,w,p,p
                x = x.reshape(shape=(imgs.shape[0], h*w, p**2 * 1))
                #x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p))
                #x = torch.einsum('nchpwq->nhwpqc', x)
                #x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 1))
            else:
                h = imgs.shape[2] // p
                w = imgs.shape[3] // p2
                #h,w = self.patch_embed.patch_hw
                x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p2))
                x = torch.einsum('nchpwq->nhwpqc', x)
                x = x.reshape(shape=(imgs.shape[0], h * w, p * p2 * 1))
        else:
            h = w = imgs.shape[2] // p
            x = imgs.reshape(shape=(imgs.shape[0], 3, h, p, w, p))
            x = torch.einsum('nchpwq->nhwpqc', x)
            x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 3))

        return x

    def unpatchify(self, x):
        """
        x: (N, L, patch_size**2 *3)
        specs: (N, 1, H, W)
        """
        p = self.patch_embed.patch_size[0]    
        h = 1024//p
        w = 256//p  #128//p
        x = x.reshape(shape=(x.shape[0], h, w, p, p, 1))
        x = torch.einsum('nhwpqc->nchpwq', x)
        specs = x.reshape(shape=(x.shape[0], 1, h * p, w * p))
        return specs

    def generate_mask_new(self, x, formant, keep_prob, device, keep_prob0=0.25):
        
        N, L, D = x.shape
        batch, seq_len = formant.shape
        
        # 计算formant中0和1的数量
        zeros_count = (formant == 0).sum(dim=1)  # [batch], formant=0的数量（随机保留）
        ones_count = formant.sum(dim=1)          # [batch], formant=1的数量（mask）
        # breakpoint()
        
        # len_keep_per_batch = (zeros_count * keep_prob0).int()  # [batch]
        # # 取所有batch中最小的保留数量作为统一的len_keep
        # len_keep = len_keep_per_batch.min().item()

        len_keep_zeros_batch = (zeros_count * keep_prob0).int() # [batch]
        len_keep_ones_batch = (ones_count * keep_prob).int()    # [batch]
        K0 = len_keep_zeros_batch.min().item()
        K1 = len_keep_ones_batch.min().item()
        len_keep = K0 + K1
        
        # 生成随机噪声，只在formant=1的位置进行随机排序
        noise = torch.rand(batch, seq_len, device=device)
        # formant=0的位置设为随机（确保排在前面，始终被保留）
        # formant=1的位置设为1(全部mask)
        # noise = 1 * formant + noise * (1 - formant)
        # 对于 formant == 0 的组：将其余位置设为无穷大，使其排在最后
        noise_zeros = noise.masked_fill(formant == 1, float('inf'))
        rank_zeros = torch.argsort(torch.argsort(noise_zeros, dim=1), dim=1)

        # 对于 formant == 1 的组：将其余位置设为无穷大
        noise_ones = noise.masked_fill(formant == 0, float('inf'))
        rank_ones = torch.argsort(torch.argsort(noise_ones, dim=1), dim=1)

        # 5. 生成区分四种状态的布尔掩码 (Mask Conditions)
        is_kept_zero   = (formant == 0) & (rank_zeros < K0)
        is_kept_one    = (formant == 1) & (rank_ones < K1)
        is_masked_zero = (formant == 0) & ~is_kept_zero
        is_masked_one  = (formant == 1) & ~is_kept_one

        # 6. 构建全新的 Sorting Key (new_noise)
        # 通过加上不同的偏置 (Offset)，我们强行控制 argsort 后的物理排布顺序：
        # 保留的0 [0,1) < 保留的1 [1,2) < 被mask的0 [2,3) < 被mask的1 [3,4)
        offset = (
            0.0 * is_kept_zero.float() +
            1.0 * is_kept_one.float() +
            2.0 * is_masked_zero.float() +
            3.0 * is_masked_one.float()
        )
        new_noise = noise + offset
        # breakpoint()
        
        ids_shuffle = torch.argsort(new_noise, dim=1)
        ids_restore = torch.argsort(ids_shuffle, dim=1)

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        # generate the binary mask: 0 is keep, 1 is remove
        mask = torch.ones([N, L], device=x.device)
        mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        mask = torch.gather(mask, dim=1, index=ids_restore)
        
        return x_masked, mask, ids_restore, ids_keep

    def random_masking(self, x, mask_ratio, formant_mask_tensor):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        """
        x_masked_f, mask_f, ids_restore_f, ids_keep = self.generate_mask_new(x=x, formant=formant_mask_tensor,
                                         keep_prob=(1 - mask_ratio),
                                         device=x.device)

        # breakpoint()

        return x_masked_f, mask_f, ids_restore_f, ids_keep
    
    def random_masking_raw(self, x, mask_ratio):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        """
        N, L, D = x.shape  # batch, length, dim
        len_keep = int(L * (1 - mask_ratio))
        
        noise = torch.rand(N, L, device=x.device)  # noise in [0, 1]
        
        # sort noise for each sample
        ids_shuffle = torch.argsort(noise, dim=1)  # ascend: small is keep, large is remove
        ids_restore = torch.argsort(ids_shuffle, dim=1)

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        # generate the binary mask: 0 is keep, 1 is remove
        mask = torch.ones([N, L], device=x.device)
        mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        mask = torch.gather(mask, dim=1, index=ids_restore)

        return x_masked, mask, ids_restore, ids_keep

    def random_masking_2d(self, x, mask_t_prob, mask_f_prob):
        """
        2D: Spectrogram (msking t and f under mask_t_prob and mask_f_prob)
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        """
        N, L, D = x.shape  # batch, length, dim
        if self.use_custom_patch: # overlapped patch
            T=101
            F=12
        else:            
            T=64
            F=8
        #x = x.reshape(N, T, F, D)
        len_keep_t = int(T * (1 - mask_t_prob))
        len_keep_f = int(F * (1 - mask_f_prob))

        # noise for mask in time
        noise_t = torch.rand(N, T, device=x.device)  # noise in [0, 1]
        # sort noise for each sample aling time
        ids_shuffle_t = torch.argsort(noise_t, dim=1) # ascend: small is keep, large is remove
        ids_restore_t = torch.argsort(ids_shuffle_t, dim=1) 
        ids_keep_t = ids_shuffle_t[:,:len_keep_t]
        # noise mask in freq
        noise_f = torch.rand(N, F, device=x.device)  # noise in [0, 1]
        ids_shuffle_f = torch.argsort(noise_f, dim=1) # ascend: small is keep, large is remove
        ids_restore_f = torch.argsort(ids_shuffle_f, dim=1) 
        ids_keep_f = ids_shuffle_f[:,:len_keep_f] #

        # generate the binary mask: 0 is keep, 1 is remove
        # mask in freq
        mask_f = torch.ones(N, F, device=x.device)
        mask_f[:,:len_keep_f] = 0
        mask_f = torch.gather(mask_f, dim=1, index=ids_restore_f).unsqueeze(1).repeat(1,T,1) # N,T,F
        # mask in time
        mask_t = torch.ones(N, T, device=x.device)
        mask_t[:,:len_keep_t] = 0
        mask_t = torch.gather(mask_t, dim=1, index=ids_restore_t).unsqueeze(1).repeat(1,F,1).permute(0,2,1) # N,T,F
        mask = 1-(1-mask_t)*(1-mask_f) # N, T, F

        # get masked x
        id2res=torch.Tensor(list(range(N*T*F))).reshape(N,T,F).to(x.device)
        id2res = id2res + 999*mask # add a large value for masked elements
        id2res2 = torch.argsort(id2res.flatten(start_dim=1))
        ids_keep=id2res2.flatten(start_dim=1)[:,:len_keep_f*len_keep_t]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        ids_restore = torch.argsort(id2res2.flatten(start_dim=1))
        mask = mask.flatten(start_dim=1)

        return x_masked, mask, ids_restore


    def forward_encoder(self, x, mask_ratio, audio, mask_2d=False):

        B = x.shape[0]
        # ssl = self.vesper(audio)
        with torch.no_grad():
            # breakpoint()
            audio_list = audio.tolist()
            input_values = self.feature_extractor(audio_list, sampling_rate=16000, padding=True, return_tensors="pt").input_values
            ssl = self.model(input_values.cuda(), output_hidden_states=True)
        # with torch.no_grad():
            # ssl = self.emo2vec.model.extract_features(audio)
        weighted_output = ssl.hidden_states[1:]

        stacked_states = torch.stack(weighted_output, dim=1)
        norm_weights = F.softmax(self.layer_weights, dim=-1).cuda()
        reshaped_weights = norm_weights.view(1, self.num_layers, 1, 1)
        weighted_output = (stacked_states * reshaped_weights).sum(dim=1)

        zeros = torch.zeros(B, 1, 1024, device=weighted_output.device, dtype=weighted_output.dtype)
        x_ssl = torch.cat([weighted_output, zeros], dim=1)
        # breakpoint()
        # ssl = ssl['x']
        
        # zeros = torch.zeros(B, 1, 768, device=ssl.device, dtype=ssl.dtype)
        # x_ssl = torch.cat([ssl, zeros], dim=1)

        # Formant
        # formant_mask_tensor = (self.patchify(formant_tensor.unsqueeze(1)).sum(-1) > 0).int()
        # x_fmt = x.detach().squeeze(1).transpose(2,1)
        # formant_mask, _ = process_batch_magnitude(x_fmt)
        # # formant_mask = formant_mask.transpose(2,1)  # epoch25 add
        x_fmt = x.detach().squeeze(1).transpose(2,1)
        formant_mask, _ = process_batch_magnitude(x_fmt)
        formant_mask_tensor = (self.patchify(formant_mask.unsqueeze(1)).sum(-1) > 0).int()
        # formant_mask_tensor = (self.patchify(formant_mask.unsqueeze(1)).sum(-1) > 0).int()
        # breakpoint()

        # embed patches
        # breakpoint()
        x = self.patch_embed(x)
        x_ssl = self.patch_embed_ssl(x_ssl)

        # add pos embed w/o cls token
        x = x + self.pos_embed[:, 1:, :]
        x_ssl = x_ssl + self.pos_embed_ssl[:, 1:, :]

        # masking: length -> length * mask_ratio
        if mask_2d:
            x, mask, ids_restore = self.random_masking_2d(x, mask_t_prob=self.mask_t_prob, mask_f_prob=self.mask_f_prob)
        else:
            x, mask, ids_restore, ids_keep = self.random_masking(x, mask_ratio, formant_mask_tensor)  # 184/1024
            # x, mask, ids_restore, ids_keep = self.random_masking_raw(x, mask_ratio)
            _, _, D = x.shape
            x_ssl = torch.gather(x_ssl, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        # breakpoint()
        # append cls token
        cls_token = self.cls_token + self.pos_embed[:, :1, :]
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)

        # append cls token
        cls_token_ssl = self.cls_token_ssl + self.pos_embed_ssl[:, :1, :]
        cls_token_ssls = cls_token_ssl.expand(x_ssl.shape[0], -1, -1)
        x_ssl = torch.cat((cls_token_ssls, x_ssl), dim=1)

        for blk in self.blocks_Mag:
            x = blk(x)
            # breakpoint()
        
        # x_idx = x.shape[1]
        # x_concat = torch.cat([x, x_ssl], dim=1)
        x_concat = torch.cat([x, x_ssl], dim=2)
        # weights = F.softmax(self.logits, dim=0)
        # x_concat = weights[0] * x + weights[1] * x_ssl
        
        # apply Transformer blocks
        for blk in self.blocks1:
            x_concat = blk(x_concat)

        x_concat = self.dowmsample(x_concat)

        for blk in self.blocks2:
            x_concat = blk(x_concat)

        x_concat = self.norm(x_concat)
        #emb = self.encoder_emb(x)

        return x_concat, mask, ids_restore, stacked_states #, x_idx #, layer_feats

    def forward_encoder_no_mask(self, x):
        # embed patches
        x = self.patch_embed(x)

        # add pos embed w/o cls token
        x = x + self.pos_embed[:, 1:, :]

        # masking: length -> length * mask_ratio
        #x, mask, ids_restore = self.random_masking(x, mask_ratio)

        # append cls token
        cls_token = self.cls_token + self.pos_embed[:, :1, :]
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)

        # apply Transformer blocks
        contextual_embs=[]
        for n, blk in enumerate(self.blocks):
            x = blk(x)
            if n > self.contextual_depth:
                contextual_embs.append(self.norm(x))
        #x = self.norm(x)
        contextual_emb = torch.stack(contextual_embs,dim=0).mean(dim=0)

        return contextual_emb

    def forward_decoder(self, x, ids_restore):
        # embed tokens
        # x = x[:,:x_idx,:]
        x = self.decoder_embed(x)
        # fbank = self.decoder_embed(fbank)

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1] + 1 - x.shape[1], 1)
        x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
        x = torch.cat([x[:, :1, :], x_], dim=1)  # append cls token

        # # append mask tokens to sequence
        # mask_tokens_fbank = self.mask_token_fbank.repeat(fbank.shape[0], ids_restore_fbank.shape[1] + 1 - fbank.shape[1], 1)
        # fbank_ = torch.cat([x[:, 1:, :], mask_tokens_fbank], dim=1)  # no cls token
        # fbank_ = torch.gather(fbank_, dim=1, index=ids_restore_fbank.unsqueeze(-1).repeat(1, 1, fbank.shape[2]))  # unshuffle
        # fbank = torch.cat([fbank[:, :1, :], fbank_], dim=1)  # append cls token

        # add pos embed
        x = x + self.decoder_pos_embed
        # fbank = fbank + self.decoder_pos_embed_fbank
        
        if self.decoder_mode != 0:
            B,L,D=x.shape
            x = x[:,1:,:]
            # fbank = fbank[:,1:,:]
            if self.use_custom_patch:
                x = x.reshape(B,101,12,D)
                x = torch.cat([x,x[:,-1,:].unsqueeze(1)],dim=1) # hack
                x = x.reshape(B,1224,D)
        if self.decoder_mode > 3: # mvit
            x = self.decoder_blocks(x)
        else:
            # apply Transformer blocks
            for blk in self.decoder_blocks:
                x = blk(x)
                # fbank = blk(fbank)
        x = self.decoder_norm(x)
        # fbank = self.decoder_norm(fbank)

        # predictor projection
        pred = self.decoder_pred(x)
        # pred_fbank = self.decoder_pred_fbank(fbank)

        # remove cls token
        if self.decoder_mode != 0:
            if self.use_custom_patch:
                pred = pred.reshape(B,102,12,256)
                pred = pred[:,:101,:,:]
                pred = pred.reshape(B,1212,256)
            else:
                pred = pred
                # pred_fbank = pred_fbank
        else:
            pred = pred[:, 1:, :]
            # pred_fbank = pred_fbank[:, 1:, :]
        return pred, None, None #emb, emb_pixel
    
    def synthesize_from_magnitude_phase(self, log_magnitude, phase, fft_size, hop_size, win_length, window='hann', center=True, onesided=True, dc_component=None):   

        magnitude = torch.exp(log_magnitude)

        complex_spec = torch.polar(magnitude, phase)

        restored_spectrum = torch.cat([dc_component, complex_spec], dim=1)
        
        audio = torch.istft(
            restored_spectrum,
            n_fft=fft_size,
            hop_length=hop_size,
            win_length=win_length,
            window=torch.hann_window(win_length).to("cuda"),
            center=center,
            normalized=False,
            onesided=onesided,
            length=None
        )
        return audio
    
    def total_loss(self, input_wav, output_wav, sample_rate=16000):
    
        relu = torch.nn.ReLU()
        l1Loss = torch.nn.L1Loss(reduction='mean')
        l2Loss = torch.nn.MSELoss(reduction='mean')
        # Collect losses as defined in paper for use with balancer
        # l_t - L1 distance between the target and compressed audio over the time domain
        # l_f - linear combination between the L1 and L2 losses over the mel-spectrogram using several time scales
        # l_g - adversarial loss for the generator
        # l_feat - relative feature matching loss for the generator
        l_t = torch.tensor([0.0], device='cuda', requires_grad=True)
        l_f = torch.tensor([0.0], device='cuda', requires_grad=True)

        #time domain loss, output_wav is the output of the generator
        l_t = l1Loss(input_wav, output_wav) 

        #frequency domain loss, window length is 2^i, hop length is 2^i/4, i \in [5,11]. combine l1 and l2 loss
        for i in range(8, 12): #e=5,...,11
            fft = Audio2Mel(n_fft=2 ** i,win_length=2 ** i, hop_length=(2 ** i) // 4, n_mel_channels=64, sampling_rate=sample_rate)
            l_f = l_f + l1Loss(fft(input_wav), fft(output_wav)) + l2Loss(fft(input_wav), fft(output_wav))

        return l_t, l_f / 4

    def forward_loss(self, imgs, pred, mask, audio, dc_component, phase, norm_pix_loss=False):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        mask: [N, L], 0 is keep, 1 is remove, 
        """
        # print("imgs", imgs)
        # print("pred", pred)
        # breakpoint()
        pred_mag = self.unpatchify(pred)
        pred_audio = self.synthesize_from_magnitude_phase(pred_mag.squeeze(1).transpose(2, 1), phase, 512,
                                              160,
                                              512,
                                              "hann_window",
                                              center=True,
                                              onesided=True,
                                              dc_component = dc_component)
        
        TAP_loss_l2 = self.TAPloss(pred_audio, audio, "train")
        TAP_loss_weight_l2 = self.TAPloss2(pred_audio, audio, "train")
        losses_restruct_t, losses_restruct_f = self.total_loss(pred_audio, audio)

        target = self.patchify(imgs)
        if norm_pix_loss:
            mean = target.mean(dim=-1, keepdim=True)
            var = target.var(dim=-1, keepdim=True)
            target = (target - mean) / (var + 1.e-6)**.5

        loss = (pred - target) ** 2
        loss = loss.mean(dim=-1)  # [N, L], mean loss per patch

        loss = (loss * mask).sum() / mask.sum()  # mean loss on removed patches


        return loss, TAP_loss_l2, TAP_loss_weight_l2, losses_restruct_t, losses_restruct_f, pred_audio

    def get_distill_loss(self, student_hidden_states, teacher_hidden_states):
        total_loss = F.mse_loss(
                    F.normalize(student_hidden_states, p=2, dim=-1),
                    F.normalize(teacher_hidden_states, p=2, dim=-1),
                )
        return total_loss
    def cal_loss(self, pred, target1, apply_norm=True):
        if apply_norm:
            loss = F.mse_loss(F.normalize(pred), F.normalize(target1))
        else:
            loss = F.mse_loss(pred, target1)
        return loss
    def forward(self, imgs, audio, dc_component, phase, mask_ratio=0.8):
        emb_enc, mask, ids_restore, ssl = self.forward_encoder(imgs, mask_ratio, audio, mask_2d=self.mask_2d)

        pred, _, _ = self.forward_decoder(emb_enc, ids_restore)  # [N, L, p*p*3]
        # breakpoint()
        loss_recon, TAP_loss_l2, TAP_loss_weight_l2, losses_restruct_t, losses_restruct_f, pred_audio = self.forward_loss(imgs, pred, mask, audio, dc_component, phase, norm_pix_loss=self.norm_pix_loss)
        loss_contrastive = torch.FloatTensor([0.0]).cuda()
        # breakpoint()    #F.mse_loss(ssl, emo2vec_tensor)
        # breakpoint()

        return loss_recon, TAP_loss_l2, TAP_loss_weight_l2, losses_restruct_t, losses_restruct_f, pred, mask, loss_contrastive


def mae_vit_small_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=384, depth=12, num_heads=6,
        decoder_embed_dim=512, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=768, depth=8, num_heads=12,
        decoder_embed_dim=512, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16,
        decoder_embed_dim=512, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def mae_vit_huge_patch14_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=14, embed_dim=1280, depth=32, num_heads=16,
        decoder_embed_dim=512, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

# set recommended archs
mae_vit_base_patch16 = mae_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_large_patch16 = mae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_huge_patch14 = mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_small_patch16 = mae_vit_small_patch16_dec512d8b # decoder: 512 dim, 8 blocks
