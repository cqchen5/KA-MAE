...
from dataset_ft import AudiosetDataset, DistributedWeightedSampler, DistributedSamplerWrapper
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='output_ft_dir',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='output_ft_dir',
    ...
    parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["audioset", "esc50", "speechcommands", "k400", "iemocap"])
...
def main(args):
    ...
    if not args.audio_exp:
        ...
        target_length = {'audioset':1024, 'esc50':512, 'speechcommands':128, 'iemocap':1024}
        multilabel_dataset = {'audioset': True, 'esc50': False, 'k400': False, 'speechcommands': True, 'iemocap': False}
    ...
    # breakpoint()
    if args.audio_exp:
        img_size=(target_length[args.dataset],256) # 1024, 128
        in_chans=1
        emb_dim = 768
    ...
    if args.finetune:
        ...
        if not args.eval:
            # for k in ['head.weight', 'head.bias', 'pre_net.weight', 'pre_net.bias', 'post_net.weight', 'post_net.bias']:
        ...
        if not args.eval:
            ...
            # trunc_normal_(model.pre_net.weight, std=2e-5)
            # trunc_normal_(model.post_net.weight, std=2e-5)
    ...
    if args.use_soft:
        ...
        criterion = nn.CrossEntropyLoss(ignore_index=-1) #nn.BCEWithLogitsLoss() # works better
    ...
    if args.eval:
        ...
        with open('metrics.txt', 'w') as fp:
            w_acc=str(test_stats['weighted_acc'])
            fp.write(f"weighted_acc:{w_acc}/n")
            uw_acc=str(test_stats['unweighted_acc'])
            fp.write(f"unweighted_acc:{uw_acc}/n")
            w_f1=str(test_stats['weighted_f1'])
            fp.write(f"weighted_f1:{w_f1}/n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.first_eval_ep:
            test_stats = evaluate(data_loader_val, model, device, args.dist_eval)
            # print(f"mAP of the network on the {len(dataset_val)}")
            if max_wa < test_stats["weighted_acc"]:
                misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch)

                max_wa = test_stats["weighted_acc"]
                print(f'Max WA: {max_wa:.4f}')
        else:
            test_stats ={'weighted_acc': 0.0, 'unweighted_acc': 0.0, 'weighted_f1': 0.0}
            print(f'too new to evaluate!')

        if log_writer is not None:
            log_writer.add_scalar('perf/weighted_acc', test_stats['weighted_acc'], epoch)
            log_writer.add_scalar('perf/unweighted_acc', test_stats['unweighted_acc'], epoch)
            log_writer.add_scalar('perf/weighted_f1', test_stats['weighted_f1'], epoch)
