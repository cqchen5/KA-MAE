...
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, use_fbank=False, fbank_dir=None, roll_mag_aug=False, load_video=False, mode='train'):
        ...
        self.load_data_info(self.datapath)
    ...
    def load_data_info(self, data_path):
        self.data = []
    ...
    def random_crop_audio_length(self, x):
        ...
        if p > 0:
            ...
            start = np.random.randint(0, x.shape[-1] - time_var)
    ...
    def _wav2fbank(self, filename, filename2=None):
        if filename2 == None:
            try:
                ...
                sr = 16000
                waveform = torch.zeros(1, 2048)

            if waveform.shape[1] <= 2048:
                channels = waveform.shape[0]
                waveform = torch.zeros((channels, 2048), dtype=waveform.dtype)
        ...
        # mixup
            ...
            try:
                ...
                print(f"Unexpected error with {filename}: {e}")
                sr = 16000
                waveform1 = torch.zeros(1, 2048)

            try:
                waveform2, _ = torchaudio.load(filename2)
            except Exception as e:
                print(f"Unexpected error with {filename2}: {e}")
                waveform2 = torch.zeros(1, 2048)

            if waveform1.shape[1] <= 2048:
                channels = waveform1.shape[0]
                waveform1 = torch.zeros((channels, 2048), dtype=waveform1.dtype)
                
            if waveform2.shape[1] <= 2048:
                channels = waveform2.shape[0]
                waveform2 = torch.zeros((channels, 2048), dtype=waveform2.dtype)
        ...
        #     fbank = torch.nan_to_num(fbank, nan=0.0)
        #     print("fbank has nan!!!")
        # print(fbank)
        ...
        # print(multi_channel_audios_stft)

        multi_channel_audios_phase = torch.angle(multi_channel_audios_stft)
        # print(multi_channel_audios_mag.shape)
        # breakpoint()
        
        return multi_channel_audios_mag.squeeze(0), 0, multi_channel_audios_crop.squeeze(0), dc_component.squeeze(0), multi_channel_audios_phase.squeeze(0)
    ...
    def __getitem__(self, index):
        ...
        if random.random() < self.mixup: # for audio_exp, when using mixup, assume multilabel
            ...
            if not self.use_fbank:
                ...
                fbank, mix_lambda = self._fbank(datum, mix_datum)
        # if not do mixup
        else:
            datum = self.data[index]
            # label_indices = np.zeros(self.label_num)
            if not self.use_fbank:
                Mag, mix_lambda, audio, dc_component, phase = self._wav2fbank(datum)
            else:
                fbank, mix_lambda = self._fbank(datum)

        # SpecAug for training (not for eval)
        freqm = torchaudio.transforms.FrequencyMasking(self.freqm)
        timem = torchaudio.transforms.TimeMasking(self.timem)

        Mag = Mag.transpose(0,1).unsqueeze(0) # 1, 128, 1024 (...,freq,time)
        if self.freqm != 0:
            Mag = freqm(Mag)
        if self.timem != 0:
            Mag = timem(Mag) # (..., freq, time)
        Mag = torch.transpose(Mag.squeeze(), 0, 1) # time, freq
        # fbank = (fbank - self.norm_mean) / (self.norm_std * 2)
        if self.noise == True: # default is false, true for spc
            Mag = Mag + torch.rand(Mag.shape[0], Mag.shape[1]) * np.random.rand() / 10
            Mag = torch.roll(Mag, np.random.randint(-10, 10), 0)

        return Mag.unsqueeze(0), audio, dc_component, phase
